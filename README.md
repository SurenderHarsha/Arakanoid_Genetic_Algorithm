...
AI has been used by Google Deepmind and OpenAI to solve many different game environments with superhuman skills. In this project we use the Arkanoid Game environment and attempt to solve its many varying levels using Genetic Algorithms. The game is run on an emulator and the scripting language used is Lua. The algorithm trains for a few hours and solves each level by normal playthrough or finding shortcuts.
...
This game can be played using Neural Networks,Reinforcement Learning etc. Using Genetic Algorithms for this task is difficult due to the presence of enemy AI and other constraints. But it can still overcome the challenges it faces by trial and error.
...
We select the top **cr_rate** (variable name) percentile of the population for crossover, the remaining are deleted/overwritten.
We have a **mutation_rate** (variable name) chance of mutating a bit in a member of population.

The fitness is calculated as `100-(NumberOfBlocksLeft/MaxBlocks)*100`.
...
This may sound like fixed movement in an unpredictable environment. The fact is that the enemy AI movement can actually be predicted as it generally depends on the player movement itself and the genetic algorithm can learn to destroy the enemy or avoid it.

A new modification to the chromosome is done, in the beginning its size is 15 (**no_controls**, variable name). But such limited amount of controls will allow us to play the game only for 20x15 = 300 Frames only. That is only 5 seconds of the game.

The project was initially written with around a 1000 control bits, completely random. But during crossover, there were two problems, the solution space either became completely random, or fixated at a local minima.
Instead I wanted to it to retain its best features and then explore new features from there. 

### Algorithm
First, the initial population of 200 members play the game individually and based on their perfomance their fitness is stored with them.
...
**Important**
In crossover, the top performers are selected for crossover, In the first generation they have input control size of 15, since they are the top performers they must have good starting points and starting strategies, therefore to these top members we add a few more random control bits of length **control_gap** (Eg 5). Then for the next generation the size of total control bits for each member is 20. The reason for this is, the best performing part of the chromosome is preserved and new random options are explored from this point onwards.These first top performers are added to the population.
 
The old top performers are taken again and instead of adding new random control bits, we add the starting control bits of other top performers(This ensures that valid moves and behaviour are also passed onto the next generation which might converge to a solution faster).
...
In Mutation, we iterate throught the members of population and through each bit of a member, with a chance of mutating that bit to its opposite value. The chance of mutation is given by **mutation_rate**. We keep it as low as possible.
